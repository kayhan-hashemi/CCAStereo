...
def unfold_3d(input_array, kernel_size, dilation=1):

    b, c, d, h, w = input_array.shape
    k = kernel_size**2
    out = unfold3d(input_array, kernel_size=(1,kernel_size,kernel_size), padding="same", 
               stride=1, dilation=dilation).view(b,c,k,d,h,w)
    return out


def unfold_2d(input_array, kernel_size, dilation=1):
    
    b, c, h, w = input_array.shape
    k = kernel_size**2
    out = unfold2d(input_array, kernel_size=(kernel_size,kernel_size), 
                   padding="same", stride=(1,1), dilation=(dilation,dilation)).view(b,c,k,h,w)
    return out
